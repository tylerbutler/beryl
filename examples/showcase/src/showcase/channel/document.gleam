...
////
//// The same behavior the standalone collab_document server implements with
//// raw app-side dispatch (`collab_document/app`), written as a
...
pub fn channel(application_context: Context) -> channel.Handler {
  channel.handler(document_registration_pattern, fn(join_context) {
    case join_context.parameters {
      [suffix] ->
        case string.split(suffix, ":") {
          [tenant, document] ->
            case payload.string_field(join_context.payload, "token") {
              Error(_) -> channel.reject(error("missing_token"))

              Ok(token) ->
                case
                  auth.verify_tenant(token, tenant, application_context.secret)
                {
                  Error(_) -> channel.reject(error("unauthorized"))
                  Ok(Nil) -> open(application_context, tenant, document)
                }
            }
...
fn open(
  ...
  let document_key = document_app.build_document_key(tenant, document)
...
fn stored_state(context: Context, document_key: String) -> Json {
  case document_store.get_state(context.store, document_key) {
    ...
    Error(document_store.Timeout) -> {
      io.println_error(
        "[showcase.channel.document] document_store.get_state timed out for key="
        <> document_key,
      )
      json.null()
    }
