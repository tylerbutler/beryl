...
pub fn channel(ctx: Ctx) -> channel.Handler {
  channel.handler(document_registration_pattern, fn(_info, topic, join_payload) {
    ...
    case beryl_topic.extract_wildcards(pattern, topic) {
      Ok([tenant, document]) ->
        case payload.string_field(join_payload, "token") {
...
fn open(ctx: Ctx, tenant: String, document: String) -> channel.JoinResult(Nil) {
  ...
  channel.accept_with(
    channel.joined(State(document_key: document_key), callbacks(ctx)),
    json.object([
  ...
  |> channel.on_message(fn(state: State, message: channel.Message) {
    case message.event {
        ...
        case payload.string_field(message.payload, "state") {
          Error(_) ->
            channel.continue_with(
              state,
              channel.actions()
                |> reply.ok(message.reply, error("invalid_state")),
            )
            ...
            case string.byte_size(encoded) > max_state_bytes {
              True ->
                channel.continue_with(
                  state,
                  channel.actions()
                    |> reply.ok(message.reply, error("state_too_large")),
                )
      ...
      _ ->
        channel.continue_with(
          state,
          channel.actions() |> reply.ok(message.reply, error("unknown_event")),
        )
    }
  })
}
