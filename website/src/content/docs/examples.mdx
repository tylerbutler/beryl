...
| Server → Client | `phx_reply` (push ref) | Reply to a client push — used for both delivery acknowledgment (`status: "ok"`) and validation errors (`status: "error", response: {code, error}`). Validation errors are returned via a `ReplyOk` effect carrying an error payload, not pushed as a separate event. |
