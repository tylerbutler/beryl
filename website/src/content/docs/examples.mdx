---
title: Examples
description: Four runnable beryl demos — realtime Todo, collaborative cursors, multi-room chat, and CRDT documents.
---
...
**Source**: [`examples/todo`](https://github.com/tylerbutler/beryl/tree/main/examples/todo)

A Lustre Todo app whose supervised BEAM actor is the source of truth. Open it
in two browser windows to see additions, toggles, and deletes synchronize.
...
pnpm build
gleam run
# Open http://localhost:8011 in multiple browser windows
```
...
|---|---|
| **Fixed topic routing** | Every browser joins the `todos` topic through app dispatch |
| **Join snapshots** | The join reply replaces the client's list with the actor's current state |
| **Canonical replies and broadcasts** | Mutations return and broadcast the server-assigned Todo or deleted ID |
| **Server validation** | Empty text, malformed payloads, and unknown IDs receive explicit error replies |
| **Reconnect synchronization** | The Phoenix client rejoins automatically and applies a fresh snapshot |
| **Typed Lustre client** | Gleam decodes every dynamic Phoenix payload before updating the UI |

### Architecture

```text
Browser (Lustre + official Phoenix JS client)
  │  WebSocket (Phoenix wire protocol)
Server (Gleam)
  ├── Mist HTTP — serves the production Lustre assets
  ├── beryl app dispatch — fixed "todos" topic
  └── supervised actor — in-memory Todos and monotonic IDs
```
...
| **`socket.RejectJoin`** | Rooms reject joins when full (20-user cap) or when room doesn't exist |
...
| Server → Client | `phx_reply` (push ref) | Reply to a client push — used for both delivery acknowledgment and validation errors. The demo returns validation failures with `socket.ReplyOk`, so their wire status is still `ok`; the response payload (`{code, error}`) represents the domain validation error. |
...
| I want a typed Lustre client with server-authoritative state | [examples/todo](https://github.com/tylerbutler/beryl/tree/main/examples/todo) |
