...
pattern matching, pipelines, and `Result` values. Familiarity with OTP
supervision and WebSocket client-server concepts will also help. You do not
need prior Phoenix Channels experience.

If beryl's model of state, inputs, and effects is new to you, start with the
[Build a live poll tutorial](/tutorial/). It explains the core model step by
step, then shows how the channel layer builds on it.
        ...
        "new_msg" ->
          channel.next(
            ...
            [
              channel.broadcast(
                "new_msg",
                // `dynamic_to_json` fails on a payload that is too deeply
                // nested to re-encode. Send `null` rather than drop the frame.
                result.unwrap(
