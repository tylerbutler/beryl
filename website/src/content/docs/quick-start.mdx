...
- **`socket.ReplyOk` / `socket.ReplyError`** — send `phx_reply` back to the calling client on the original ref.
- **`socket.Push`** — send a named event to this socket only, on a topic it already joined.
- **`socket.Broadcast`** — send a named event to every socket on the topic, including the sender.
- **`socket.Next(model, [])`** — continue with no outgoing frames.
</Aside>
...
when a ref is present and return `socket.ReplyOk`:

```gleam
socket.Message(topic, "new_msg", payload, Some(ref)) ->
  socket.Next(
    model,
    [
      socket.Broadcast(topic, "new_msg", wire.dynamic_to_json(payload)),
...
Return `socket.RejectJoin` when a topic should not be admitted:
