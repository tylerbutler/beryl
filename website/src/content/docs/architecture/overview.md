...
The app supplies an `init`/`update` pair to `beryl.start_app`; the runtime actor dispatches decoded wire messages to it as typed events and applies the effects it returns.
...
|---|---|---|
| `beryl` | Public entry-point: `config/1`, `start_app/3`, `broadcast/4`, `broadcast_from/5`, `stop/1` | — |
...
| `beryl/rate_limit` | Token-bucket rate limiter; keyed per socket, per socket+topic, or per topic pattern | — |
| `beryl/log` | Internal logging shim over `palabres`; thin named-logger surface, not public API | — |
| `beryl/internal` | Shared internal utilities (logging config, crash rescue); not public API | — |
...
The runtime supervises itself inside `start_app`; presence and groups are plain actors the application starts and owns. See the [Supervision guide](/guides/supervision/).
