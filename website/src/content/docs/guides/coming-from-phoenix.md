...
| `terminate/2` | `socket.Closed(topic, reason)` event, delivered on every exit path |
...
| `Phoenix.Presence.track/3` / `untrack/3` | `socket.PresenceTrack(topic, key, meta)` / `socket.PresenceUntrack(topic, key)` effects |
| `push(socket, "presence_state", Presence.list(socket))` | `socket.PushPresence(topic, "presence_state", presence_wire.encode_state)` |
...
socket.Join(topic_name, _payload, ref) ->
  socket.Next(model, [
    ...
    socket.PushPresence(topic_name, "presence_state", presence_wire.encode_state),
  ])
```
