...
1. **`InvalidPattern(pattern, reason)`** — the pattern is not a valid beryl topic pattern; `reason` is the core `beryl/topic.TopicError` (`EmptyTopic` or `InvalidFormat(detail)`), nested so you can match on it. New variants may be added in a minor release, so use a catch-all (`InvalidPattern(pattern, _)`) unless you need the exact one. Check the pattern against `beryl/topic` syntax: `"room:lobby"`, `"room:*"`, `"document:*:ops"`, `"*"`. Empty patterns are rejected.
