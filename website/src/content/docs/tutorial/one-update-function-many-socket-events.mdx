...
labels effect execution separately from the recipients' deliveries.

The same rule applies to a join. If one update returns
`[AcceptJoin(ref, None), Push(topic, "ready", payload)]`, the join
acknowledgment reaches the wire before the push. The runtime drops a push to a
topic that is not joined yet.
...
The demo starts after both clients have joined and fetched `get_state`.
`AcceptJoin(ref, None)` acknowledges a join without totals. The reply to
`get_state` supplies them. Each displayed private model is a subset of
`Model(sender, topics)`, with the sender omitted; the client display and shared
poll totals are separate state.

Vote as each client. The [ordered effect list](#effects-run-in-list-order)
replies to the voter, then broadcasts to the other subscribed socket.
Disconnect one client and reconnect it while the other remains. Its new
socket model fetches the existing totals. Disconnect both, then reconnect:
the last-member cleanup removed the old room, so the new poll starts at zero.
The store actor remains alive throughout. This cleanup is application policy,
not a beryl persistence guarantee.

The controls disconnect a whole socket. Leaving only `poll:demo` would end
that subscription without closing its connection. Chapter 4 demonstrates
that difference. The local simulation prepares you for the two real browser
tabs in the checkpoint below; it does not replace them.
