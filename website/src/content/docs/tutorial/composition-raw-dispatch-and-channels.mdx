...
import TopicCompositionDemo from '../../../components/TopicCompositionDemo.astro';
...
<TopicCompositionDemo />

Vote, send a guide tip, then leave the poll and send another tip. The
connection and guide subscription remain active. Switch the explanatory view
at any point: the poll and guide counts do not change, and no join or message
runs because you changed views.
...
[`tutorial_composition.gleam`](https://github.com/tylerbutler/beryl/blob/main/examples/live_poll/test/tutorial_composition.gleam).

This demo has one poll member, so leaving the poll removes its room. Rejoining
starts at zero.
...
This is effect execution order, not an arrival-order promise across different
client connections.
