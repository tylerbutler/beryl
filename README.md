...
do not mitigate this vector** — all three run post-assembly, so the buffer
grows within a single admitted connection and before any event reaches a
socket's `update` function.
