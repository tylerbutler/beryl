...
////
////   * `beryl.start` with a hand-written `update`, and
////   * `beryl_channels.start` with a handler table.
////
...
pub fn variants() -> List(Variant) {
  [
    Variant(name: "beryl.start", start: fn(config) {
      let assert Ok(sockets) =
        ...
        as "the raw contract system starts"
      sockets
    }),
    Variant(name: "beryl_channels.start", start: fn(config) {
      let assert Ok(sockets) =
        beryl_channels.start(config, handlers: handlers())
        as "the channel contract system starts"
      sockets
    }),
  ]
}
