...
// to 0 and silently disable eviction. `start_app` rejects timeouts below 2.

pub fn start_app_rejects_timeout_of_one_test() {
  start_trivial(1)
  |> should.equal(Error(beryl.InvalidHeartbeatTimeout))
}
...
fn start_trivial(timeout_ms: Int) -> Result(beryl.Channels, beryl.StartError) {
  beryl.start_app(
    beryl.config(wire.phoenix_codec())
      |> beryl.with_heartbeat(interval_ms: 30_000, timeout_ms: timeout_ms),
    init: fn(_info: event.ConnectInfo(Nil)) { #(Nil, []) },
