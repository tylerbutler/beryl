    ...
    h.start_app(
      ...
      update: fn(model, ev) {
        ...
        case ev {
          ...
          Message(_topic, "stop", _payload, _ref) -> Stop(event.Normal)
          Closed("room:b", event.Shutdown) -> Next(model, [KickTopic("room:c")])
          _ -> Next(model, [])
        }
      },
    )
...
pub fn kick_closes_topic_and_leaves_others_test() {
  ...
  let assert Ok(Closed("room:b", event.Shutdown)) = process.receive(events, 500)
...
pub fn kick_chain_from_closed_terminates_test() {
  ...
  let assert Ok(Closed("room:b", event.Shutdown)) = process.receive(events, 500)
  let assert Ok(Closed("room:c", event.Shutdown)) = process.receive(events, 500)
...
pub fn stop_tears_down_socket_and_calls_closer_test() {
  ...
  let assert Ok(Closed("room:b", event.Normal)) = process.receive(events, 500)
