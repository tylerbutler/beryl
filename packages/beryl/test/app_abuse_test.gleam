...
//// (via the `app_test_helper` wrappers), the same path any transport uses
...
pub fn join_rate_limit_rejects_excess_joins_test() -> Nil {
...
pub fn message_rate_limit_sheds_flood_test() -> Nil {
  let events = process.new_subject()
  let config =
    beryl.config(wire.phoenix_codec())
...
pub fn joins_do_not_consume_message_rate_token_test() -> Nil {
  ...
  app_test_helper.push(channels, "s1", "room:a", "message", "r-1")
  let assert Ok(Message("room:a", "message", _, _)) =
    process.receive(events, 500)

  // The second message spends the token the joins never touched.
  app_test_helper.push(channels, "s1", "room:b", "message", "r-2")
  process.receive(events, 100) |> should.be_error
}

pub fn invalid_decoded_event_consumes_message_rate_token_test() -> Nil {
  ...
  app_test_helper.push(channels, "s1", "room:a", "message", "r-good")
  process.receive(events, 200) |> should.be_error
}

pub fn direct_route_decoded_enforces_message_rate_test() -> Nil {
  ...
  // same runtime message-rate enforcement as the `app_test_helper.route`
...
pub fn topic_cap_rejects_excess_join_test() -> Nil {
  let events = process.new_subject()
  let config =
    beryl.config(wire.phoenix_codec())
...
pub fn channel_bucket_cap_bounds_distinct_topics_test() -> Nil {
  let events = process.new_subject()
  let channels = app_test_helper.start_observed(capped_config(), events)
  let frames = app_test_helper.connect(channels, "s1")
  ...
  app_test_helper.push(channels, "s1", "room:a", "m", "r-a")
  let assert Ok(Message("room:a", _, _, _)) = process.receive(events, 500)
  app_test_helper.push(channels, "s1", "room:b", "m", "r-b")
  let assert Ok(Message("room:b", _, _, _)) = process.receive(events, 500)
  app_test_helper.push(channels, "s1", "room:c", "m", "r-c")
  process.receive(events, 200) |> should.be_error
}

pub fn channel_bucket_cap_recovers_after_leave_test() -> Nil {
  let events = process.new_subject()
  let channels = app_test_helper.start_observed(capped_config(), events)
  let frames = app_test_helper.connect(channels, "s1")
  join_ok(channels, events, frames, "s1", "room:a", "jr-a")
...
pub fn channel_bucket_cap_is_isolated_per_socket_test() -> Nil {
  ...
  app_test_helper.push(channels, "s1", "room:b", "m", "r-b")
  let assert Ok(Message("room:b", _, _, _)) = process.receive(events, 500)
  app_test_helper.push(channels, "s2", "room:x", "m", "r-x")
  let assert Ok(Message("room:x", _, _, _)) = process.receive(events, 500)
  app_test_helper.push(channels, "s2", "room:y", "m", "r-y")
...
pub fn unjoined_events_do_not_consume_channel_bucket_cap_test() -> Nil {
  ...
  let frames = app_test_helper.connect(channels, "s-sec")
  ...
  app_test_helper.push(channels, "s-sec", "room:real", "client_event", "r-real")
