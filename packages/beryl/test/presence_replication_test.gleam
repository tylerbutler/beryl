...
pub fn restarted_node_presences_replicate_to_peers_test() -> Nil {
  ...
  let assert Ok(p1) = presence.start(test_config(pubsub_instance, "node1", 30))
  let assert Ok(p2) = presence.start(test_config(pubsub_instance, "node2", 30))
  ...
  let assert Ok(p1b) = presence.start(test_config(pubsub_instance, "node1", 30))
