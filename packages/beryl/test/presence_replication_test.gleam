...
pub fn two_replicas_converge_via_pubsub_test() -> Nil {
  let pubsub_instance = test_pubsub("converge_2")

  let config1 = test_config(pubsub_instance, "node1", 50)
  ...
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker1, "room:lobby")) == 2 },
    2000,
    20,
  )
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker2, "room:lobby")) == 2 },
    2000,
    20,
  )

  let entries1 = presence_entries(tracker1, "room:lobby")
  let entries2 = presence_entries(tracker2, "room:lobby")
...
pub fn self_broadcast_ignored_test() -> Nil {
  let pubsub_instance = test_pubsub("self_bcast")
...
pub fn remote_state_triggers_merge_via_pubsub_test() -> Nil {
  let pubsub_instance = test_pubsub("remote_merge")
  ...
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker1, "room:lobby")) == 1 },
    2000,
    20,
  )

  // Node1 should now see node2's entry via PubSub replication
  let entries = presence_entries(tracker1, "room:lobby")
  list.length(entries) |> should.equal(1)
...
pub fn remote_merge_updates_read_model_count_test() -> Nil {
  let pubsub_instance = test_pubsub("remote_merge_count")
...
pub fn three_replicas_converge_test() -> Nil {
  let pubsub_instance = test_pubsub("converge_3")

  let config1 = test_config(pubsub_instance, "node1", 50)
  let config2 = test_config(pubsub_instance, "node2", 50)
  let config3 = test_config(pubsub_instance, "node3", 50)
  ...
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker1, "room:lobby")) == 3 },
    2000,
    20,
  )
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker2, "room:lobby")) == 3 },
    2000,
    20,
  )
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker3, "room:lobby")) == 3 },
    2000,
    20,
  )
  ...
  let entries1 = presence_entries(tracker1, "room:lobby")
  let entries2 = presence_entries(tracker2, "room:lobby")
  let entries3 = presence_entries(tracker3, "room:lobby")
...
pub fn presence_without_pubsub_still_works_test() -> Nil {
...
pub fn untrack_propagates_via_pubsub_test() -> Nil {
  let pubsub_instance = test_pubsub("untrack_prop")
  ...
  test_helper.wait_until(
    fn() { list.length(presence_entries(tracker2, "room:lobby")) == 1 },
    2000,
    20,
  )
  ...
  test_helper.wait_until(
    fn() { presence_entries(tracker2, "room:lobby") == [] },
    2000,
    20,
  )
...
pub fn restarted_node_presences_replicate_to_peers_test() -> Nil {
  let pubsub_instance = test_pubsub("restart_join")
  let assert Ok(tracker1) =
...
pub fn restart_prunes_previous_incarnations_ghosts_test() -> Nil {
  let pubsub_instance = test_pubsub("restart_ghosts")
  let assert Ok(tracker1) =
    presence.start(test_config(pubsub_instance, "node1", 30))
  let assert Ok(tracker2) =
    presence.start(test_config(pubsub_instance, "node2", 30))
  ...
  test_helper.wait_until(
    fn() {
      ...
      let on_restarted_tracker1 =
        presence_entries(restarted_tracker1, "room:lobby")
        |> list.map(fn(entry) { entry.key })
      on_tracker2 == ["user:live"] && on_restarted_tracker1 == ["user:live"]
    },
    3000,
    10,
  )
...
pub fn restart_prune_updates_read_model_count_test() -> Nil {
  let pubsub_instance = test_pubsub("restart_prune_count")
  let assert Ok(tracker1) =
    presence.start(test_config(pubsub_instance, "node1", 30))
  let assert Ok(tracker2) =
    presence.start(test_config(pubsub_instance, "node2", 30))
...
pub fn reads_stay_responsive_while_actor_mailbox_is_blocked_test() -> Nil {
...
pub fn actor_reply_is_ordered_after_read_model_publication_test() -> Nil {
  let entered = process.new_subject()
  let config =
    presence.default_config("node1")
