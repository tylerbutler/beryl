...
/// A start_app system that accepts every join and (optionally) forwards
/// handled `Message` events to an observer.
fn start_accepting_app(config: beryl.Config) -> beryl.Channels {
  start_observed_app(config, process.new_subject())
}

fn start_observed_app(
  config: beryl.Config,
      ...
      update: fn(model, ev) {
        case ev {
          ...
          Message(_topic, event_name, _payload, _ref) -> {
            process.send(handled, event_name)
            Next(model, [])
          }
          _ -> Next(model, [])
        }
...
pub fn join_with_control_character_topic_gets_error_reply_test() {
  ...
  let frames = h.connect(channels, "socket-ctrl-join")

  // Topic contains a newline — rejected before reaching the app.
  h.route(
    channels,
    "socket-ctrl-join",
    "[null,\"ref-1\",\"room:\\nlobby\",\"phx_join\",{}]",
...
pub fn join_with_too_long_topic_gets_error_reply_test() {
  ...
  let frames = h.connect(channels, "socket-long-topic")
...
pub fn join_topic_over_byte_limit_but_under_grapheme_limit_gets_error_reply_test() {
  ...
  let frames = h.connect(channels, "socket-multibyte-topic")
...
pub fn event_over_byte_limit_but_under_grapheme_limit_is_dropped_test() {
  ...
  let frames = h.connect(channels, "socket-multibyte-event")

  h.join(channels, "socket-multibyte-event", "room:lobby", "ref-1", "ref-1")
  ...
  // Sentinel event: the runtime processes messages in order, so if the
...
pub fn socket_cannot_join_more_than_configured_topic_cap_test() {
  ...
  let frames = h.connect(channels, "socket-join-cap")
  ...
  let second_reply = h.recv(frames)
...
pub fn channels_handle_remains_usable_after_start_test() {
  ...
  // The opaque handle keeps working for broadcasts after start.
  beryl.broadcast(channels, "opaque:lobby", "noop", json.object([]))
  |> should.equal(Nil)

  beryl.stop(channels)
}

pub fn stop_shuts_down_supervised_runtime_test() {
  let config =
    beryl.config(wire.phoenix_codec())
  ...
  let assert Ok(runtime_pid) = beryl.app_runtime_pid(channels)
  process.is_alive(runtime_pid) |> should.be_true

  let frames = h.connect(channels, "socket-stop")
  h.join(channels, "socket-stop", "stop:lobby", "ref-stop", "ref-stop")
  let _join_reply = h.recv(frames)

  beryl.stop(channels)
  wait_until(fn() { !process.is_alive(runtime_pid) }, 1000, 10)

  process.is_alive(runtime_pid) |> should.be_false
