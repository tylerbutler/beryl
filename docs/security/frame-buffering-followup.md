...
This document tracks the upstream part of GitHub issue #198,
...
- `src/beryl/transport/mist.gleam:445-463`: `on_message` measures
  ...
  emits a complete `Text`/`Binary` message, after buffering and aggregation.
...
- `build/packages/mist/src/mist/internal/websocket.gleam:133-157`: for each
  inbound TCP chunk, it calls
  ...
  ...)`). No limit applies to `state.buffer`. The buffer grows until a
  frame completes.
...
- `build/packages/gramps/src/gramps/websocket.gleam:85-160`: `decode_frame`
  ...
  length, and then pattern-matches `payload:bytes-size(payload_byte_size)`.
  When the complete declared payload is not available, it returns
...
- `build/packages/gramps/src/gramps/websocket.gleam:451-474`:
  ...
  (BitArray concatenation) with **no cumulative size limit**. Therefore, many
  fragmented frames can form one unbounded buffer.
...
1. **Upstream a size cap into gramps/mist (preferred).**
   - Add a configurable `max_frame_bytes` (or reuse a WebSocket-specific limit)
     ...
     after it parses the header. Return an error (which causes a
...
2. **Pin a patched fork of gramps/mist.**
   - Carry the size-cap patch in a fork pinned via `gleam.toml` until the
     upstream change is released. This option has a higher maintenance cost.
     It is an acceptable temporary solution if the memory limit must ship
     before the upstream projects merge the change.
...
3. **Mandate an edge-proxy / load-balancer frame-size limit (shipped now).**
   ...
   - This is operational, not in-process, and depends on correct deployment
     configuration. It is the only available mitigation today.
...
Still OPEN. These criteria require the upstream dependency change:
